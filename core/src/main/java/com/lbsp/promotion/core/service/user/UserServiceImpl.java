...
public class UserServiceImpl extends BaseServiceImpl<User> implements
	...
	public UserRsp getUserByAccountandPassword(String account, String password) {
		...
		/** 查询出用户基本信息 */
		User tUser = userDao.getUserByAccount(account,
				Params.GenericStatus.ENABLED);
	...
	private void setUserRsp(UserRsp rsp, User user) {
		...
		if (GenericConstants.LBSP_ADMINISTRATOR_ID == user.getId()) {
			...
			List<OperateResource> userPage = privilegeService
					.findEnabledPageByMasterandAccess(
							GenericConstants.LBSP_PRIVILEGE_MASTER_TYPE_USER,
							userIdParams);
			List<OperateResource> rolePage = privilegeService
					.findEnabledPageByMasterandAccess(
							GenericConstants.LBSP_PRIVILEGE_MASTER_TYPE_ROLE,
							roleIdParams);
		...
		if (GenericConstants.LBSP_ADMINISTRATOR_ID == user.getId()) {
			List<FunctionOperate> funcList = functionOperateService
					.allFunctions();
			...
			for (FunctionOperate func : funcList) {
				...
				functions.add(or);
				if (func.getUrl().indexOf("{") == -1
						&& func.getUrl().indexOf("}") == -1) {
			...
			List<OperateResource> userFunc = privilegeService
					.findEnabledFuncByMasterandAccess(
							GenericConstants.LBSP_PRIVILEGE_MASTER_TYPE_USER,
							userIdParams);
			List<OperateResource> roleFunc = privilegeService
					.findEnabledFuncByMasterandAccess(
							GenericConstants.LBSP_PRIVILEGE_MASTER_TYPE_ROLE,
							roleIdParams);
		...
		/*
		 * //保存用户与角色映射名称 List<Operate> mapOper =
		 * operateDao.getOperateRoleMapName(roles); Map<String,String> mapOpers
		 * = new LinkedHashMap<String, String>(); for (Operate o : mapOper){
		 * String tmp = null; if(mapOpers.get(o.getCode()) == null){ tmp =
		 * o.getName(); }else{ tmp = mapOpers.get(o.getCode()); tmp +=
		 * ","+o.getName(); } mapOpers.put(o.getCode(),tmp); }
		 * rsp.setMapRoleOperName(mapOpers); //以String的形式保存操作权限 List<String>
		 * operStrList = new ArrayList<String>(); for (Operate o : operList){
		 * operStrList.add(o.getCode()); } rsp.setOperates(operStrList);
		 * rsp.setOperatesAll(operList); //加载权限前缀
		 * rsp.setOperPrefix(operateDao.getOperPrefix());
		 * rsp.setOperOtherPrefix(operateDao.getOtherOperPrefix());
		 * 
		 * //查询用户所有的接口访问路径 List<String> interUrls = new ArrayList<String>();
		 * List<Interface> operInters = new ArrayList<Interface>();
		 * Map<String,Integer> paramInter = new HashMap<String, Integer>();
		 * //如果用户是超级管理员(系统自带生成的唯一的根用户)，自动装载所有接口服务URL if
		 * (GenericConstants.LBSP_ADMINISTRATOR_ID.equals(user.getId())){
		 * operInters = interfaceService.getResourceUrlByOperate(null); }else {
		 * //如果非超级管理员，查询其所拥有的接口服务URL operInters =
		 * interfaceService.getResourceUrlByOperate(rsp.getOperatesAll()); }
		 * for(Interface interfaces : operInters){
		 * if(interfaces.getUrl().indexOf("{") != -1 &&
		 * interfaces.getUrl().indexOf("}") != -1){ String url =
		 * interfaces.getUrl(); int start = url.indexOf("{"); int len =
		 * url.substring(start).split("/").length;
		 * paramInter.put(url.substring(0,start-1),len); }else{
		 * interUrls.add(interfaces.getUrl()); } } rsp.setInterList(interUrls);
	...
	public boolean addUser(User user) {
		...
		param.put(new QueryKey("account", QueryKey.Operators.EQ),
				user.getAccount());
		param2.put(new QueryKey("username", QueryKey.Operators.EQ),
				user.getUsername());
		...
		if (ulst.size() <= 0 && ulst2.size() <= 0) {
			...
			// 添加全体共有角色、修改角色用户数
			// roleDao.addUserRole(user.getId(),
			// "000000000002"/*UserServiceImpl.ELS_WEB_ALL_ACCOUNT_ID*/, now,
			// user.getCreate_user());
			Role role = roleService.findOne("000000000002"/*
														 * UserServiceImpl.
														 * ELS_WEB_ALL_ACCOUNT_ID
														 */);
